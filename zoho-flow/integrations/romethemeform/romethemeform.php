 ...
 * @since romethemeform     1.2.0
 */
class Zoho_Flow_RomethemeForm extends Zoho_Flow_Service{
    ...
    public function list_form_fields( $request ){
        ...
        if( $this->is_valid_form( $form_id ) ){
            $form_fields = get_post_meta( $form_id, '_elementor_data' );
            return rest_ensure_response( json_decode( $form_fields[0], true ) );
        }
    ...
    private function is_valid_form( $form_id ){
        if( isset( $form_id ) ){
            if( "romethemeform_form" === get_post_type( $form_id ) ){
                return true;
            }
            return false;
        }
     ...
     * @param   bool        $update     Update check flag.
     */
    public function payload_form_entry_submitted( $post_id, $post, $update ){
        $form_id = get_post_meta( $post_id, 'rform-entri-form-id' );
        if( $form_id[0] ){
            $args = array(
            ...
            if( !empty( $webhooks ) ){
                $event_data = array(
                    ...
                    'data' => array(
                        'entry_id' => $post_id,
                        'form_id' => $form_id[0],
                        'entry_data' => json_decode( get_post_meta( $post_id, 'rform-entri-data' )[0], true),
                        'referal_post_id' => get_post_meta( $post_id, 'rform-entri-referal' )[0]
                    )
                );
    ...
    public function get_system_info(){
        ...
        $plugin_dir = ABSPATH . 'wp-content/plugins/romethemeform/rometheme-form.php';
        if(file_exists( $plugin_dir ) ){
            $plugin_data = get_plugin_data( $plugin_dir );
            $system_info['romethemeform'] = $plugin_data['Version'];
        }
