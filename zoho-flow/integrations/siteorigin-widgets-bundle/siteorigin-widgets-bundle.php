 ...
 * @since siteorigin-widgets-bundle 1.64.2
 */
class Zoho_Flow_SiteOrigin_Widgets_Bundle extends Zoho_Flow_Service{
    ...
    public function list_forms( $request ){
        ...
        foreach( $posts as $post ){
            $panel_data = get_post_meta( $post->ID, 'panels_data', true );
            if( isset( $panel_data ) ){
                $forms = $this->getFormWidgets( $panel_data['widgets'], $post->ID, $post->post_title );
                if( !empty( $forms ) ){
    ...
    private function getFormWidgets( $widgets, $post_id, $post_name ) {
        $formWidgets = [];
    
        foreach ($widgets as $widget) {
            if ( isset($widget['_sow_form_id']) ) {
                $formWidgets[] = [
                    'title' => $widget['title'],
                    'form_id' => $widget['_sow_form_id'],
                    'post_id' => $post_id,
                    'post_name' => $post_name
                ];
            }
            if( isset( $widget['panels_data'] ) ){
                $formWidgets = array_merge( $formWidgets, $this->getFormWidgets( $widget['panels_data']['widgets'], $post_id, $post_name ) );
            }
        }
    ...
    public function list_form_fields( $request ){
        ...
        $panel_data = get_post_meta( $post_id, 'panels_data', true );
        if( $panel_data ){
            $fields = $this->getFormFieldsById( $panel_data['widgets'], $form_id );
            if( $fields ){
    ...
    function getFormFieldsById($widgets, $form_id) {
        foreach ($widgets as $widget) {
            if (isset($widget['_sow_form_id']) && $widget['_sow_form_id'] === $form_id) {
                return $widget['fields'] ?? []; // Return fields if found, otherwise return an empty array
            }
            elseif( isset( $widget['panels_data'] ) ){
                $fields = $this->getFormFieldsById( $widget['panels_data']['widgets'], $form_id );
                if( !empty( $fields ) ){
                    return $fields;
                }
            }
        }
        return []; // Return empty array if no matching form ID is found
    }
    ...
    public function create_webhook( $request ){
        ...
        if( ( isset( $entry->name ) ) && ( isset( $entry->url ) ) && ( isset( $entry->event ) ) && ( in_array( $entry->event, self::$supported_events ) ) && ( preg_match( "/\b(?:(?:https?|ftp):\/\/|www\.)[-a-z0-9+&@#\/%?=~_|!:,.;]*[-a-z0-9+&@#\/%=~_|]/i", $entry->url ) ) ){
            ...
            $post_name = "SiteOrigin Widgets Bundle ";
     ...
     */
    public function payload_form_entry_submitted( $instance, $fields_values ){
        $args = array(
    ...
    public function get_system_info(){
        ...
        $plugin_dir = ABSPATH . 'wp-content/plugins/so-widgets-bundle/so-widgets-bundle.php';
        if(file_exists( $plugin_dir ) ){
            $plugin_data = get_plugin_data( $plugin_dir );
            $system_info['siteorigin_widgets_bundle'] = $plugin_data['Version'];
        }
